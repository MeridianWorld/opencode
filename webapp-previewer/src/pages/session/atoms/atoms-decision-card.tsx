...
export function AtomsDecisionCard(props: { row: Extract<AtomsRow, { kind: "decision" }> }) {
  return (
    <article class="max-w-[40rem] rounded-[24px] border border-[var(--atoms-accent)]/30 bg-[color-mix(in_srgb,var(--atoms-card)_96%,white)] px-4 py-3 shadow-[var(--atoms-shadow-soft)]">
      <div class="text-[11px] font-semibold uppercase tracking-[0.18em] text-[var(--atoms-soft)]">
        {props.row.request === "question" ? "Input needed" : "Approval needed"}
      </div>
      ...
      <div class="mt-3 rounded-[18px] border border-dashed border-[var(--atoms-line)] px-3 py-2 text-[12px] leading-6 text-[var(--atoms-soft)]">
        Respond from the dock below to keep the build moving.
      </div>
    </article>
  )
}
